...
Because queries occupy disjoint $k$-slices, they do not conflict on router resources, yielding the $O(\log N)$ bandwidth improvement described in Section 1 at the cost of approximately twice as many routers.
...
Each qubit in the Fat-Tree QRAM is accessed via a 1D \texttt{QuantumRegister} corresponding to its role (input, route, left, or right).
We calculate the index of the qubit within this register using the following formula: $(\sum_{m=0}^{k-1}(2^{m+1}-1))+2^i-1+j$ with $k$ the tree index (with the smallest tree having $k=0$), $i$ the node level, and $j$ the 
...
As an example, the route qubit for the fourth node on the third level of the fourth tree would be indexed as \texttt{regs['router\_route\_qubit'][17]}.
...
the correct result. 
We then ran tests using the Qiskit Sampler on the IBM Quantum Platform, which gave results
that largely matched the expected output. 
However, IBM's QPUs offer at most 156 superconducting qubits, insufficient to incorporate quantum error correction, so result quality declined as QRAM size increased.

\vspace{0.2cm}

\begin{tabular}{r|c|c}
    \textbf{Size}& \textbf{Bucket Brigade} & \textbf{Fat-Tree}\\ 
    \hline 
    \hline 
    1 & 84\% & 82\%\\ 
    \hline 
...
In testing, all memory addresses were initialised to 0 except the last (set to 1), which was the query target.
As seen above, both architectures achieved similar success probabilities, with Fat-Tree showing only a slight drop due to its higher gate count.
...
and the Fat-Tree QRAM required $4\cdot (\sum_{i=1}^{n}(2^{i}-1)) = 4\cdot (2^{n+1}-n-2)$ 
qubits.
Thus, Fat-Tree QRAM's $O(\log N)$ bandwidth improvement requires fewer than twice as many qubits as BB QRAM.
...
the additional number of swap gates per round is $4\cdot(2^n-n-1)$.
For a size-$n$ Fat-Tree, a query completes in $n$ rounds, so when pipelining $m$ queries,
we have an additional $(m+n-1)\cdot 4\cdot (2^n-n-1)$ SWAP gates required.
One query in a BB QRAM requires $2\cdot(n+1)\cdot (n+2)+2\cdot n$ gates total and so the 
additional swap gates over $m$ queries gives
$O(\frac{2^n}{n\cdot m})$ as the ratio between the added swap gates and the gates that 
were present before.

This SWAP overhead is further compounded on superconducting hardware, where limited connectivity means multiple physical swaps may be required for each logical swap.
This, however, can be mitigated on alternative platforms.
Trapped-ion computers provide all-to-all connectivity within ion chains\cite{Landsman_2019}, meaning each qubit connects to many others, so fewer physical swaps are needed.
Additionally, neutral-atom architectures allow qubits to be physically repositioned using optical tweezers\cite{Beugnon_2007}, potentially eliminating SWAP gates entirely and leaving only the CSWAP operations required for routing.

%-----------------------------------------------------------------------------
\subsection{Parallel Query Performance}

The increased performance for parallel queries was immediately evident when analysing the generated circuits: multiple queries can now be in flight simultaneously within the QRAM. In the pipelined Fat-Tree schedule, a new query enters every four timesteps, so up to $n$ queries overlap during steady-state execution. This confirms the theoretical $O(\log N)$ bandwidth improvement, as $n = O(\log N)$ queries complete in $O(\log N)$ total time rather than the $O(n \cdot \log N)$ required by sequential BB QRAM queries.
...
at the cost of fewer than a 2 times increase in router qubits and a moderate rise in SWAP 
...
current superconducting QPUs suffers from significant noise-induced degradation,
