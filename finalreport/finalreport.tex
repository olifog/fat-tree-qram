...
The test suite validates correctness through simulation using Qiskit's \texttt{Sampler}, testing both classical address queries and superposition queries for various data patterns. Comparison tests verify that BB and Fat-Tree implementations produce identical results for single queries.
...
To access each qubit in the Fat-Tree QRAM, we first index the Qiskit \texttt{QuantumCircuit} with the name of the role the qubit serves in the structure to access a 1D Qiskit \texttt{QuantumRegister} that will contain all 
qubits that serve that role; namely, the roles inside the structure are input, route, left \& right.
We calculate the index of the qubit within this register using the following formula: $(\Sigma_{n=0}^{k-1}2^{n+1}-1)+2^i-1+j$ with $k$ the tree index (with the smallest tree having $k=0$), $i$ the node level \& $j$ the 
index of the node on that level.
This results in the nodes being ordered as a left-to-right, top-to-bottom trace of each tree, starting from the smallest.
As an example, the route qubit for the third node on the second level of the fourth tree would be indexed as \texttt{regs['router\_route\_qubit'][17]}.
...
To verify correctness, we traced the generated circuits for both BB \& Fat-Tree QRAM to 
ensure that as long as there are no errors from noise, the structure will always deliver 
the correct result. 
We then ran tests using the Qiskit Sampler using the IBM Quantum Platform which gave results
that largely corresponded to the desired output. 
However, due to the QPU's offered by IBM having a maximum of 156 superconducting qubits, 
we were unable to build in adequate quantum error correction to mitigate errors and so 
the quality of the results greatly declined as we increased the size of the QRAMs.
\begin{tabular}{r|c|c}
    \textbf{levels}& \textbf{BB} & \textbf{Fat-Tree}\\ 
    \hline 
    \hline 
    1 & 84\% & 82\%\\ 
    \hline 
    ...
    \hline
\end{tabular}\\
In the testing we had all memory addresses initialised to 0, apart from the last address 
which was initialised to 1 and it was this last address which we were trying to access.
As seen in the table above, the resulting probability of for each QRAM to achieve the 
correct result was largely the same with only a slight drop caused by the increased gates 
for the Fat-Tree QRAM.

%-----------------------------------------------------------------------------
\subsection{Resource Scaling}

In terms of logical qubits required to implement the routers for each tree, we found that 
BB QRAM required $4\cdot (2^{n}-1)$ qubits for a QRAM that is indexed by $n$ address qubits
and the Fat-Tree QRAM required $4\cdot (\sum_{i=1}^{n}(2^{i}-1))\approx 4\cdot (2^{n+1}-n-2)$ 
qubits.
This means that for the Fat-Tree QRAM's benefit of providing an $O(\log n)$ bandwidth 
improvement over BB QRAM, we require less than twice the amount of qubits to achieve this.

There is also an increase in the gate count as we now require SWAP gates to move queries 
between different trees in the Fat-Tree QRAM.
If we define one round to be the time between starting two different queries, we find that 
the additional number of swap gates per round is $4\cdot(2^n-n-3)$.
For an $n$ size Fat-Tree, a query completes in $n$ rounds so when pipelining $m$ queries,
We have an additional $(m+n-1)\cdot 4\cdot (2^n-n-3)$ SWAP gates required.
One query in a BB QRAM requires $2\cdot(n+1)\cdot (n+2)+2\cdot n$ gates total and so the 
additional swap gates represent an increase of approximately $\frac{2^n}{n}$.

This problem of increased SWAP gates is further compounded when we think about what they 
actually represent on a superconducting, with the high distance between qubits (due to 
there being a large number that need to interact with each other) meaning that multiple 
swaps will actually be required for each swap present in the abstracted circuit.
This however, can be mitigated if this approach to QRAM was done on trapped-ion or 
neutral-atom quantum computers.
Trapped-ion computers are able to provide all-to-all connectivity inside an ion chain\cite{Landsman_2019} 
meaning that the each qubits is connected to a larger number of other qubits, so the 
total physical swap gates required will be less.
Additionally, in neutral-atom architectures, the qubits can be moved physically using optical tweezeres\cite{Beugnon_2007} to be closer to other qubits, essentially removing the need 
for swap gates in the qram, with the only gate that the router needs being CSWAP \& inverse
CSWAP gates to do the actual routing.

%-----------------------------------------------------------------------------
\subsection{Parallel Query Performance}

The increased performance for parallel queries was immediately evident when analysing 
the generated circuits as it was clear that there could now be multiple queries in 
flight at once within the QRAM.
...
In this work, we successfully reproduced and validated the Fat-Tree QRAM architecture 
proposed by Xu et al., demonstrating its ability to support pipelined, high-bandwidth
quantum memory queries while preserving the correctness guarantees of BB QRAM. Using 
Qiskit we implemented both architectures and verified functional equivalence whilst 
confirming that Fat-Tree QRAM allows multiple concurrent queries to remain in flight 
without conflict. Our resource scaling analysis confirms that this parallelism comes 
at the cost of fewer than a 2× increase in router qubits and a moderate rise in SWAP 
overhead—costs that may not be as large a problem on hardware platforms with improved 
connectivity, such as trapped ions or neutral atoms. Although physical execution on 
current superconducting QPUs suffers from a lot of noise-induced degradation,
our circuit-level analysis supports the theoretical claim that Fat-Tree QRAM achieves an 
$O(\log N)$ bandwidth improvement over BB QRAM.
